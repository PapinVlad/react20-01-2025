...
import './slide.css'
 ...
 const Slide = () => {
    const [slideIn, slideInActive] = useState(false);

    const toggleButton = () => {
        slideInActive((prev) => !prev);
    ...
    return(
        ...
        className={`btn ${slideIn ? "open" : ""} px-5 py-2.5 flex items-center justify-center rounded-full text-white text-sm 
        tracking-wider font-semibold border-none outline-none bg-orange-600 hover:bg-orange-700 
        active:bg-orange-600`}>
        {slideIn ? "Back to Dashboard" : "View Details"}
        ...
        <div className={`element ${slideIn ? "open" : ""}`}> I'm the div that slides in </div>
        </>
    )
